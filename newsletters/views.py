...
def newsletter_signUp(request):
    ...
    if form.is_valid():
        ...
        if NewsletterUser.objects.filter(email=instance.email).exists():
            ...
            instance.save()
            messages.success(request, 'Hemos enviado un correro electronico a su correo, abrelo para continuar con el entrenamiento')
            #correo electrónico
...
def newsletter_unsubscribe(request):
    ...
    if form.is_valid():
        ...
        if NewsletterUser.objects.filter(email=instance.email).exists():
            ...
            messages.success(request,'Email has been removerd.')
        else:
